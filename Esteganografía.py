# -*- coding: utf-8 -*-
"""
Created on Tue Feb  8 20:17:46 2022
...
class app():
    def __init__(self):
        self.window = Tk()
        self.window.title("Esteganografía")
        ...
        self.btnClear = Button(self.window,text="LIMPIAR TEXTO",width=20,bg=self.backgr,command=self.clear)
        self.btnClear.place(x=5,y=277)
        self.rdbEncode = Radiobutton(self.window,text="Codificar",variable=self.mode,value="EN",command=self.set_mode)
        self.rdbEncode.place(x=420,y=277)
        self.rdbDecode = Radiobutton(self.window,text="Decodificar",variable=self.mode,value="DE",command=self.set_mode)
        self.rdbDecode.place(x=506,y=277)
        self.btnSearch = Button(self.window,text="BUSCAR",width=20,bg=self.backgr,command=self.open_file)
        ...
        self.btnStart = Button(self.window,text="EJECUTANDO",width=81,bg=self.backgr,command=self.init_task)
        self.btnStart.place(x=5,y=353)
        self.bylab = Label(self.window,text="BYTES DISPONIBLES:")
    ...
    def open_file(self):
        file = filedialog.askopenfilename(initialdir="/",title="Selecciona una imagen de tipo .PNG",
        ...
        if file != "":
            ...
            try:
                ...
                messagebox.showwarning("ERROR","No se puede abrir el archivo.")
    ...
    def encode(self):
        ...
        if len(secret_data) <= self.n_bytes:
            ...
            messagebox.showinfo("COMPLETADO","Imagen Creada: {}".format(ima_name))
        else:
            messagebox.showwarning("ERROR","Insuficiencia de bytes, se necesita una imagen mas grande o menor data.")
    ...
    def decode(self):
        ...
        if "=====" in decoded_data:
            ...
            messagebox.showwarning("AVISO","No existe data oculta.")
        self.invLabel.configure(text="")
        self.running = False
    ...
    def init_task(self):
        if self.file_name != "":
            if self.mode.get()=="EN" and len(self.textEntry.get('1.0',END))>1 and self.running==False:
                self.running = True
                self.invLabel.configure(text="CODIFICACION...")
                t = threading.Thread(target=self.encode)
                t.start()
            elif self.mode.get()=="DE" and self.running==False:
                self.running = True
                self.invLabel.configure(text="DECODIFICACION...")
                t = threading.Thread(target=self.decode)
                t.start()
        else:
            messagebox.showwarning("NINGUN ARCHIVO","Selecciona una imagen.")
